...
- If you use the @sys-number entity as a node condition and the user specifies zero as the value, the 0 value is recognized properly as a number. However, in the condition, the 0 is interpreted as `null`, which evaluates to false, which means the node is not processed. To check for numbers in a way that handles zeros properly, use the expression `@sys-number >= 0` in the node condition instead. (Ignore any messages about an incorrect entity operator; the operator works.)
