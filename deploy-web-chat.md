...
When security is enabled, your assistant uses RSA Signature with SHA-256 to encrypt communication. RS256 cryptography is a sophistocated type of RSA encryption. An RSA key pair includes a private and a public key. The RSA private key is used to generate digital signatures, and the RSA public key is used to verify digital signatures. The complexity of the RSA algorithm that is used to scramble the message makes it nearly impossible to unscramble the message without the key.
...
1.  Use your private key to sign a JSON Web Token (JWT). You will pass the token with the messages that are sent from your web site as proof of their origin.
    ...
    Most programming languages offer JWT libraries that you can use to generate a token. For a code sample that illustrates how to use NodeJS to generate a JWT token, see [Token generation](https://integrations.us-south.assistant.watson.cloud.ibm.com/web/developer-documentation/api-instance-methods#token_generation){: external}.
...
1.  To prove that a message is coming from your website, each message that is submitted from your Web Chat implementation must include the JSON Web Token (JWT) that you created earlier.
    ...
    For more information, see [Token generation](https://integrations.us-south.assistant.watson.cloud.ibm.com/web/developer-documentation/api-instance-methods#token_generation){: external}.
