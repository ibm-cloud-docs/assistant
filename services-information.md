...
The service uses the following information from API requests for billing purposes:
  ...
  - **user_id**: A property defined in the V2 API that is sent in the context object of a /message API call. Using this property is the best way to ensure that you accurately attribute /message API calls to unique users. See the [API reference documentation]( https://console.bluemix.net/apidocs/assistant-v2#send-user-input-to-assistant) for more information about where this is defined (context>global>system>user_id).
