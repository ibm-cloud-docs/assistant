...
- [Cómo evitar confusiones con números](#dialog-slots-avoid-number-confusion)
...
- [Cómo evitar que se muestre una respuesta Encontrado cuando no es necesario](#dialog-slots-stifle-found-responses)
...
El uso de `@sys-number` en una condición de ranura resulta útil para capturar cualquier número que especifiquen los usuarios en sus entradas. Sin embargo, el comportamiento no es el esperado cuando los usuarios especifican el número cero (0). En lugar de tratar el cero como un número válido, la condición se evalúa como falsa y el servicio solicita de nuevo un número al usuario. Para evitar este comportamiento, compruebe si hay `@sys-number` o `@sys-number:0` en la condición de ranura.

Para asegurarse de que una condición de ranura que comprueba menciones de números trate los ceros correctamente, siga estos pasos:

1.  Añada `@sys-number || @sys-number:0` al campo de condición de ranura y especifique el nombre de la variable de contexto y el texto de la indicación.
1.  Pulse el icono **Editar respuesta** ![Editar respuesta](images/edit-slot.png).
1.  Pulse el menú **Más** ![Icono Más](images/kabob.png) y seleccione **Abrir editor de JSON**.
1.  Actualice la variable de contexto que ahora tiene la sintaxis `"number":"@sys-number || @sys-number:0"` para especificar solo `@sys-number`.
...
En la lógica exclusiva de la característica de ranuras, cuando dos entidades del sistema se reconocen como una sola entrada de usuario, se utiliza la que tiene el intervalo más largo. Por ejemplo, si el usuario especifica *May 2*, aunque el servicio de {{site.data.keyword.conversationshort}} reconoce las entidades @sys-date (05022017) y @sys-number (2) en el texto, solo se registra y se aplica a la ranura la entidad del sistema con un mayor rango (@sys-date).
{: tip}

Para cada entidad que se reconoce en la entrada de usuario, solo se puede cumplimentar una ranura. Por lo tanto, si tiene dos ranuras que buscan valores parecidos, colóquelas de modo que la ranura que captura la serie más larga esté encima de la que captura la serie más corta. Por ejemplo, si una ranura captura un ID de producto (`@id`) con una sintaxis como `GR1234` y la otra captura un número (`@number`), como por ejemplo `1234`, coloque la ranura que captura el ID por encima de la que captura el número. De lo contrario, cuando la entrada del usuario contenga un ID, como por ejemplo `BR3344`, la ranura `@number` podría reclamarlo como una referencia de número y cumplimentar la variable de contexto `$number` con `3344`. Sin embargo, es más probable que el valor sea una referencia de ID de producto que se debería guardar en la variable de contexto `$id` de la ranura `@id` como `BR3344`.
