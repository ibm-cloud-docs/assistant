...
1.  Monitor the logs from conversations that users have with the deployed assistant.
    ...
    You can view the logs for a version of a skill that is running in production from the Improve tab of a development version of the skill. As you find misclassifications or other issues, you can correct them in the development version of the skill, and then deploy the improved version to production after testing. See [Improving across assistants](/docs/services/assistant?topic=assistant-logs#logs-deploy-id) for more details.
