...
| `@sys-date` | `day_of_week` | Evaluates the date to determine which day of the week it is. Stores the day of the week in lower case. For example, `Sunday` is stored as `sunday`. Use a lower-case initial letter to check for a weekday name in a condition. For example: `@sys-date.day_of_week == 'sunday'` |
